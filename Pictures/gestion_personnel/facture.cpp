...
//{return cin_prop; }
/*QSqlQueryModel* Facture::getcins33()
{


    QSqlQueryModel* model=new QSqlQueryModel();

    model->setQuery("SELECT CIN FROM GESTIONCLIENT");
...
/*Facture* Facture::getcin20(int client)
{
    QSqlQuery query;
    query.prepare("SELECT * FROM GESTIONCLIENT WHERE CIN = :CIN");
    query.bindValue(":CIN",client);
    query.exec();
    Facture * F = new Facture();
    while (query.next()) {
            //F->setcin(query.value(0).toString()) ;
