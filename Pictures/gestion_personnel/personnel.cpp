...
bool personnel::modifier_emp(int cin_emp ,QString nom_emp, QString prenom_emp,QString  id_emp,QString parking_emp, int tel_emp,int nbr_conge_emp,QString etat_emp,QString Specialite_emp,QDate daate_emp)
{
    bool test=false;
    QSqlQuery query;
    query.prepare("UPDATE PERSSONEL SET CINEMP= :CINEM,NOMEMP= :NOMEMP,PRENOMEMP= :PRENOMEMP,IDEMP= :IDEMP, PARKINGEMP= :PARKINGEMP, TELEMP= :TELEMP,NBR_CONGEEMP= :NBR_CONGEEMP,ETATEMP= :ETATEMP,SPECIALITEEMP= :SPECIALITEEMP,DAATEEMP= :DAATEEMP where CINEMP= :CINEMP ");
